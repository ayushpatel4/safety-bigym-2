...
**Phase 0/0.5/1/2 done; P3.0 (Phase 3 scaffolding) done and merged. Workstream D (CQN-AS demo pipeline) code-complete + unit-tested + 2000-frame GPU-box smoke PASSED (2026-05-20); one validation run remains, then Phase 3 (P3.1).**
...
- [x] B5. SVF training on v1 dataset — proximity-trained safety critic landed; checkpoint at `checkpoints/svf_coworker_train_v1.pt` (627k). Headline Phase 2 deliverable.
  ...
  - [ ] B5.5 **OPEN (2026-05-20)** — in-dist eval confirmed v1 critic narrowness (residual 74–87% at 28–34% intervention on `coworker_train`). Triggering the **snapshot action denormalization** fix from B4.2 + v2 dataset collection. Pipeline: [`scripts/run_phase2_b55.sh`](../scripts/run_phase2_b55.sh) (4 stages: collect → train → eval → sweep, gated by `STAGES` env var, `--smoke` first). Full plan + acceptance criteria in [phase2_results.md §B5.5](phase2_results.md#7--b55--v2-dataset-with-snapshot-action-denormalization-active). Prerequisite: the `_SnapshotPolicy` patch (sketch in the script header) — the script's first action is a source-grep that aborts if it isn't in `main`. Outputs land at `datasets/svf_coworker_train_v2/`, `checkpoints/svf_coworker_train_v2.pt`, `results/svf_eval_v2_{cell}.csv`, `results/svf_sweep_v2_{task}.csv`.
...
- [ ] D3b-validation. ~30–50k-frame trained run with demos + `env.safety.add_workspace_penalty=true` showing task attempts (single bodyslam mode, oracle recommended) — **GPU-box, next**. Expected: non-trivial episode_reward > 0 on some episodes; the actor's still random at 2000 frames so the 50k run is what distinguishes "task attempted" from "well-behaved random."
