...
- **BiGym DemoStore demos are 3-dof and unusable for Phase 2** (as of 2026-05-16). Cached demos use `floating_dofs=['pelvis_x', 'pelvis_y', 'pelvis_rz']`; B2.4 moved Phase 2 env construction to 4 dofs. Metadata mismatch → `DemoNotFoundError` on every task. B3 ships without the demo source; random + snapshot must cover the safe-side distribution. Re-recording demos through the 4-dof env is a tracked follow-up, not a B3 blocker.
...
- **Phase 3 scaffolding (P3.0) is landed** (merged PR #9, 2026-05-20). The Lagrangian glue itself (P3.1: λ PID, dual-Q `argmax_a [Q_r − λ·Q_c]`, Q_c training-loop integration) is NOT done. What exists:
  - **Workspace reward shaping** — `SafetyConfig.add_workspace_penalty` / `workspace_radius` (0.4) / `workspace_beta` (0.2), applied in `SafetyBiGymEnv._reward()` via `_compute_workspace_penalty()`; threaded through `safety_bigym_factory._create_env`. Off by default.
