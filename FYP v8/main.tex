...
%
%  STRUCTURE (8 content chapters; see docs/REPORT_STRATEGY.md for rationale):
%    Ch. 1 Introduction
...
%
%  Fill-in markers:
%    \placeholder{...}: prose to be written, rendered in grey
%    \result{X}: numerical result to be filled, rendered in brackets
%    \todo{...}: outstanding action, rendered in red
%
%  Compile:
...
that animates the coworker, and to the Imperial College London
...
\label{ch:intro}
% =============================================================================
% Target: 4-5 pages. No experimental results. Motivate; state the
% gap; list the contributions; cross-reference where each contribution
% is delivered.
...
invariant holds (Theorem~\ref{lem:support-bound}). A runtime safety
...
(Section~\ref{sec:results:headline-feature-table}).
\item \textbf{A lemma on bounded reward shaping in distributional
critics} (Theorem~\ref{lem:support-bound}): for a C51 distributional
...
out future work, and Chapter~\ref{ch:conclusion} concludes.
Hyperparameter tables, a reproducibility checklist, and the
declarations on generative-AI use, ethics, sustainability, and code
availability appear as appendices.
...
\label{ch:background}
% =============================================================================
% Target: 8 pages. The criteria caution against "predominantly
% extended literature reviews." This chapter is selective, critical,
% and recent.
...
(Theorem~\ref{lem:support-bound}). Our backbone is built on a C51
...
\bigym{} tasks (we obtain around \result{85}\,\% success on
...
% =============================================================================
% Target: 5 pages. Frame as a benchmark contribution. Explain why a
% new benchmark is needed; what it tests; how it relates to BiGym.
...
Theorem~\ref{lem:support-bound} as a value-support invariant of the
...
\label{ch:method}
% =============================================================================
% Target: 12 pages. Carries the bulk of the Execution mark. Every
% major design choice (1) is specified, (2) is justified against at
% least one named alternative, (3) is cross-referenced to the
% experiment that ablates it. Framed explicitly as solving the
% problem the benchmark surfaces.
...
\item A \textbf{deployment-time mechanism}: an offline-trained,
  frozen safety value function (SVF) that intercepts the policy's
  proposed actions and vetoes catastrophically unsafe proposals
  with a fallback action (Section~\ref{sec:method:filter}).
\end{enumerate}
...
transition count for the current critic is \placeholder{N transitions};
the authoritative figures are in the Phase~2 results write-up.
...
Roughly $16.8\,\%$ of the collected transitions are proximity-violating.
A \texttt{WeightedRandomSampler} over-samples the violating class to a
$1{:}1$ ratio of safe to violating transitions per batch, so the critic
is not dominated by the safe majority.
...
\begin{theorem}[Support-bounding invariant]
...
\item[(\textbf{3}) \emph{Speed-scaling filter on baseline}] = (1)
  ...
  ($d_{\mathrm{slow}} = 0.40$\,m). Tests reactive, deployment-time
  velocity control alone (the velocity axis), without retraining.
...
Three independent seeds per evaluation cell. 20 evaluation episodes
per seed sampled from the relevant \texttt{ParameterSpace}.
95\,\% bootstrap confidence intervals on all headline metrics
(10\,000 bootstrap resamples). Significance tests reported only
where a directional claim is made: Welch's $t$-test for means,
bootstrap for \cvar{} comparisons. Effect sizes reported alongside
$p$-values.
...
\placeholder{Total $\sim$\result{73}\,GPU-hours mandatory, rising to
$\sim$\result{92}\,GPU-hours including the strengthening experiments,
on a single NVIDIA A100 (40\,GB). Mandatory breakdown: G1 base-policy
curriculum (stages 0/1/2) $\sim$\result{19}\,h; Phase~2 SVF re-eval,
retrain and sweep $\sim$\result{6}\,h; E3.1 cost-signal form ablation
$\sim$\result{18}\,h; E3.2 cost-budget Pareto sweep $\sim$\result{24}\,h;
E4.1 headline (row-2 training only, rows 1/3/4/5 are pure eval)
$\sim$\result{6}\,h; E4.3 internalisation curve $\sim$\result{1}\,h
(the benchmark harness is engineering effort, not GPU time).
Strengthening: E3.6 observation-channel under the constrained policy
$\sim$\result{7}\,h; WCSAC external baseline $\sim$\result{10}\,h;
E5.1/E5.2 robustness (pure eval) $\sim$\result{2}\,h. Carbon
$\sim$8\,kg~CO\textsubscript{2}e under the 2026 UK grid
emissions factor; derived in Appendix~\ref{app:sustainability}.}
...
\begin{tabular}{lc}
\toprule
Metric & Unconstrained baseline \\
\midrule
\texttt{success\_rate}                          & $0.85$ \\
\texttt{ep\_proximity\_violation\_rate}         & $0.296$ \\
\texttt{ep\_ssm\_violation\_actual\_rate}       & $0.146$ \\
...
\begin{table}[h]
\centering
\caption{Experiment E3.6. The constrained policy and the
unconstrained baseline re-evaluated under \texttt{oracle} and
\texttt{noisy} perception (\texttt{saucepan\_to\_hob},
\texttt{coworker\_train}, 3 seeds $\times$ 20 episodes). The policy's
proximity reduction is preserved under realistic noisy, lagged
perception (CIs across modes overlap), and the baseline shows no
oracle--noisy gap because it does not use the channel to avoid. The
headline E4.1 table reports the \texttt{noisy} column.}
...
observations). No reactive fallback reduces geometric proximity at
acceptable success and robot speed. Zero-velocity braking is a
velocity backstop only; an aggressive retreat buys separation by
abandoning the task and accelerating, which worsens the \ssm{}-actual
rate.}
...
\texttt{zero\_velocity}    & 2.25 & 15\,\% & $\approx 0.296$ & $\approx 0.85$ & $0.27$ \\
...
at $0.76$ task success, pooled over three seeds (180 episodes). Each
...
\texttt{noisy} perception, pooled over 3 seeds $\times$ 20 episodes.
The policy owns the geometric-proximity axis ($\tau = 0.3$\,m); the
graded speed-scaling filter owns the velocity-adaptive \ssm{}-actual
axis; only their composition reduces both at once, at a multiplicative
success cost. $\Delta$ columns are relative to the unconstrained
baseline (negative is safer). The two SVF-veto configurations are in
...
\midrule
Unconstrained baseline                 & $0.85$ & $0.296$ & --       & $0.146$ & --       & -- \\
Proactive policy ($\lambda{=}0.1$)     & $0.76$ & $\mathbf{0.228}$ & $\mathbf{-23\,\%}$ & $0.112$ & $-23\,\%$ & -- \\
Speed-scaling filter (on baseline)     & $0.53$ & $0.273$ & $-8\,\%$ & $\mathbf{0.048}$ & $\mathbf{-67\,\%}$ & $45\,\%$ \\
\textbf{Policy $+$ speed-scaling (hybrid)} & $\mathbf{0.44}$ & $0.250$ & $-15\,\%$ & $0.065$ & $-55\,\%$ & $45\,\%$ \\
\bottomrule
\end{tabular}
\end{table}
...
right design; binary veto is not.
...
(\coworker{}, \texttt{noisy}). Every reactive modality, learned or
model-based, is bounded by the freeze-versus-flee dilemma: it either
freezes (and proximity does not fall) or flees (and task success
collapses). None reaches the proactive policy's frontier
...
costs success. The same flee appears whichever part of the robot
...
\label{ch:disc}
% =============================================================================
% Target: 4 pages. The 20% Reflection mark band lives here.
...
\label{ch:future}
% =============================================================================
% Target: 3 pages. Specific, not generic. Each subsection is a
% concrete project that a follow-up student could pick up.
...
support-bounding invariant (Theorem~\ref{lem:support-bound}) for
...
Value support & $v_{\min} = -6$, $v_{\max} = +2$ & satisfies Thm~\ref{lem:support-bound} \\
...
($d_{\mathrm{slow}} = 0.40$\,m), not the learned SVF veto. The learned
...
\midrule
\textbf{Composed filter} & graded speed-scaling, $d_{\mathrm{slow}} = 0.40$\,m & velocity (\ssm{}-actual) axis \\
...
\placeholder{Code repository: \url{https://github.com/ayushpatel4/safety-bigym-2}.
Exact commit hashes for headline experiments. Dataset versions for
\bigym{}~\cite{chernyadev2024bigym} (\result{v0.9.0}) and
AMASS~\cite{mahmood2019amass} (\result{CMU subset, [date]}).
Hardware: single NVIDIA A100 40\,GB, CUDA \result{12.X}, PyTorch
\result{2.11}. Step-by-step recipe for reproducing the headline
Table~\ref{tab:e4.1-feature-incremental} including approximate
wall-clock time per phase. Random seeds: \result{0, 1, 2} for 3-seed
cells; \result{0, 1, 2, 3, 4} for 5-seed cells. \bigym{} version and
local patches applied (see public fork at \result{[URL]}).}
...
Total compute was \result{$\sim$92}\,GPU-hours on a single NVIDIA A100
(40\,GB) including the strengthening experiments, of which
\result{$\sim$73}\,GPU-hours are the mandatory core. At an assumed mean
board power of $0.3$\,kW and a data-centre power-usage effectiveness of
$1.4$, this is approximately $39$\,kWh; at the 2024 UK grid carbon
intensity of $0.21$\,kg\,CO\textsubscript{2}e/kWh, approximately
$8$\,kg\,CO\textsubscript{2}e (core only: $\approx$\,31\,kWh,
$\approx$\,6.5\,kg). Several practices kept this figure low: resumable
...
\placeholder{Code repository: \url{https://github.com/ayushpatel4/safety-bigym-2}, MIT licence.
Pre-trained model weights for the headline Lagrangian policy and
the Phase 2 SVF: \result{[weights URL]} (hosted on \result{[platform]}).
Dataset shards used to train the Phase 2 SVF: \result{[URL]}
(v3 snapshot-only collection on \texttt{coworker\_train} under
\texttt{noisy}; \placeholder{N transitions}, \placeholder{$\sim$Z\,GB}
compressed). \bigym{}
simulator fork (with the collision-channel and PD-actuator fixes
from Section~\ref{sec:bench:human}) released as a public fork at
\result{[URL]} with patches submitted upstream. The snapshot
evaluation harness (Section~\ref{sec:bench:harness}) is shipped as
part of the main repository under \texttt{scripts/benchmark\_policy.py}.}
