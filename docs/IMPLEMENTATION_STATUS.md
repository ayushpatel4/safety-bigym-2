...
**🔴 BASE-FIX comes first (2026-05-20).** The D3b base-validation FAILED: degenerate policy, reward −78→−775, robot retreats from the task. Root cause = the dense workspace penalty's discounted return saturates the C51 critic support [−2,+2] (the Bellman clamp kills value learning). Fix landed on `safety-critic/phase-3-constrained-rl` (bounded penalty + widened support + 36 demos + staged human curriculum). **Next action: run `scripts/run_base_curriculum.sh` on the GPU box (stage 0 is the gate).** Full writeup: [phase3_base_validation_findings.md](phase3_base_validation_findings.md). Only after a non-degenerate base do we un-park P3.1 (P3.1e smoke), then commit the branch (ask the user; never `main`) and hand E3.* sweeps to the human.
...
- [ ] **Re-validation (GPU box, next).** Run `scripts/run_base_curriculum.sh`. **Stage 0 (idle human) is the gate:** episode_reward must climb >0 on some episodes with returns inside [−6, 2] (no saturation). If stage 0 fails, the problem is demos/CQN-AS, not safety. Then stages 1→2 must keep task attempts as the human ramps in. Smoke first.
- Design invariant to preserve on any β/cap change: `β·workspace_excess_cap/(1−γ) ≤ |agent.v_min|` (here `0.05·1.0/0.01 = 5 ≤ 6`).
- [x] **C51 projection index-clamp bugfix (2026-05-21)** — bring-up of the stage-0 smoke surfaced an opaque CUDA device-side assert in the vendored `compute_target_q_dist` `index_add_`. Root cause: float32 rounding makes `ceil((v_max−v_min)/delta_z) = atoms` at the support ceiling (reward=1 success transitions), OOB by one atom; stochastic, exposed by more successful demos (29/36 vs 6/10). Fixed by clamping `b` to `[0, atoms-1]` (one-line, sanctioned edit to `agent.py`, logged in its header) — also fixes the P3.1 cost critic's same path. Added `train_cqn_as` finite-batch guard + `tests/test_c51_projection_bounds.py`. Stage-0 smoke at `num_demos=10` already ran clean to 2000 frames; re-run `num_demos=36` after pulling the fix.
