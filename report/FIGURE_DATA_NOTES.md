...
## Task-suite figure (v17 panel swap, 2026-06-12): task_suite.png

The dishwasher_close and drawers_open_all panels of the v16 figure were
replaced (user request: dishwasher's coworker was cropped out of frame;
drawers' coworker clipped through the cabinet). The saucepan panel and
all bar styling are unchanged (bars redrawn pixel-matched; see
`scripts/compose_task_suite.py --qa`).

**Provenance of the two new panels — expert-demo replay, NOT baseline
snapshots.** The baseline snapshots live only on the GPU box; per the
author's direction the replacement stills mock the snapshot rollouts:
cached BiGym expert demos (`~/.bigym/demonstrations/0.9.0/`, 4-dof
absolute) are replayed action-for-action at native 500 Hz inside the
live safety env (G1 coworker, COWORKER_PATROL, canonical stage-2
training band, `coworker_train` knobs), so the robot performs the real
task in real physics while the coworker behaves exactly as in training.
Both replayed episodes success-terminate. Per-step ground-truth
closest-pair separation is recorded in the same pass, so the annotated
"separation here" values remain env ground truth at the pictured
instant. The caption's "live rollouts of each task's unconstrained
baseline policy" was accordingly weakened to "live task executions"
(main.tex fig:task-suite).

- Pipeline: `scripts/remake_task_suite_panels.py` (replay + state
...
- v16 `task_suite_manifest.json` is retained as the record of the
  ...
  plus `task_suite_v17_update.json`.

## Task-suite figure (v16, 2026-06-11): task_suite.png [panels 2-3 superseded 2026-06-12]

Generated by `scripts/make_regime_figures.py` (new): live rollouts of each
task's unconstrained baseline policy, frames + ground-truth per-step
closest-pair separation captured in the same pass (the policy is not
bit-reproducible across runs, so a two-pass design would mismatch).

- Conditions: `coworker_train` disruption, G1 human, `oracle` obs mode,
  6 episodes per task, seeds 0-5 (`seed_base*100_000+ep`).
- Snapshots (the same baselines behind the benchmark numbers):
  - saucepan: `exp_local/cqn_as_base_curriculum/base_g1_30k_30k_40k_20260529_124749/stage2_full/snapshot_28203.pt`
    (= ROW1 per `results/e4_1/e3_6_base_oracle.csv`)
  - dishwasher: `exp_local/curriculum/dish_rung1_a/stage2_full/snapshot_best.pt`
  - drawers: `exp_local/curriculum/drawers_rung1_a/stage2_full/snapshot_best.pt`
    (both = the gated_sweep baselines per `results/gated_sweep/*.csv`)
- Chosen stills (manual pick from 344 candidates for both-agents-visible
  framing): saucepan ep0 step 200 (sep 0.33 m), dishwasher ep2 step 180
  (sep 0.22 m), drawers ep0 step 150 (sep 0.51 m). The annotated separation
  in each label bar is the env's ground-truth closest-pair value at that
  step. Full episode stats in `task_suite_manifest.json` (this dir) and
  `results/figs/regime_rollouts/manifest.json`.

**Considered and rejected: a per-step separation trace / barcode figure**
(`results/figs/regime_rollouts/regime_{traces,barcode}.{pdf,png}`). In a
6-episode sample the raw separation bands do not cleanly separate the
regimes (the saucepan sample pooled ~0.32 within 0.5 m vs the benchmark's
0.483; two dishwasher stall episodes read as persistent), because the
regime variable is the *gate-active* fraction (critic risk), not raw
separation bands, and single-episode samples are bimodal. Including it
would have muddied the regime message; the protocol-grade regime evidence
remains fig7 (gate activity). Artifacts kept for the author under
`results/figs/regime_rollouts/`.
