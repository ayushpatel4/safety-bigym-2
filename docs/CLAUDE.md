...
- **Three flavours of "ssm/proximity violation"** (as of 2026-05-26). `info["safety"]` carries `ssm_violation` (conservative ISO 15066 with `v_h = v_h_max`), `ssm_violation_actual` (velocity-adaptive ISO with observed human velocity), and `proximity_violation` (geometric `min_separation < proximity_threshold`, default 0.5 m). **The thesis primary safety axis is `proximity_violation_rate`** — ISO `S_p` at kitchen-scale robot speeds demands ~5 m clearance and over-fires `ssm_violation` (Phase 2 B2.3 saw 93 % on random rollouts). The semantics of `ssm_violation` *changed* in this revision: it used to track the observed-capped human velocity (closer to today's `ssm_violation_actual`); from 2026-05-26 it's the worst-case bound. Per-episode aggregates (`ep_proximity_violation_rate`, `ep_ssm_violation_actual_rate`, `ep_time_in_proximity_{0p3,0p5,1p0}m`, separation quantiles, robot-vel diagnostics) live on `info["episode_safety"]`. Full reference: [docs/safety_metrics.md](safety_metrics.md).
