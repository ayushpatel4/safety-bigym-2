...
success cost. The Hybrid Safety Critic is therefore a two-arm
architecture: proactive policy learning where co-location is
...
this class of system is therefore a greenfield problem, and closing it
is what this project targets.
...
\item \emph{Commercial humanoid platforms are not certified
  ...
  is happening but the certification is not.
\item \emph{Robot learning's safety record remains thin at deployment.}
  ...
  have not received proportional attention in robot-learning
  evaluation. Brunke et al.~\cite{brunke2022safelearning} and the
  newer safe-RL survey of Wachi et al.~\cite{wachi2024constraint}
  show an active field of constrained and safety-filtered learning, but
  the representative benchmarks remain low-dimensional, abstract-cost,
  or separate from manipulation with a live human coworker
  (e.g.\ Safety-Gymnasium~\cite{ji2023safetygymnasium} and
  safe-control-gym~\cite{yuan2022safecontrolgym}).
...
Commercial deployment is moving faster than published safety methods. Manipulation
benchmarks largely ignore safety, safety benchmarks largely ignore
manipulation, and safe-RL methods usually choose either a
training-time constraint or a runtime filter, not both. On top of
this, most methods assume cleaner human state than a real robot would
have. The intersection is exactly where a safe humanoid manipulator
must operate. It is also largely uncharted: recent work covers parts of
the problem, but not the full combination of high-DOF humanoid
manipulation, a moving human stand-in, \iso{}-derived separation and
force signals, noisy human-state perception, and a direct comparison of
training-time and runtime safety mechanisms. This project enters that
intersection by closing three concrete gaps in safe-RL evaluation for
human-co-located humanoid manipulation:
...
\item \textbf{No published work characterises \emph{when} each
  ...
  Each family reports settings where it works, but neither offers a rule
...
\emph{simultaneously}. None of the pieces has an off-the-shelf
...
critic must be trained on the same noisy perception distribution it
...
\item[RQ2] Can a \emph{value-based} Lagrangian constrained-RL policy
  ...
  collapsing the task?
\item[RQ3] Which runtime safety filter should be used at deployment?
  Specifically, when should the robot stop, slow down, move away, or
  use a learned critic to decide when to slow down?
\item[RQ4] What decides which safety mechanism works? Does the pattern
  of human--robot co-location explain why one task needs proactive
  policy learning while another can be handled by a runtime filter?
\end{description}
...
parts of that space, but not the whole setting at once. The
contributions are therefore both infrastructural and empirical:

\begin{enumerate}[label=\textbf{C\arabic*.}, leftmargin=2em]
\item \textbf{A benchmark for asking the safety question.}
  \sbigym{} extends \bigym{} with a moving Unitree G1 coworker, three
  \iso{}-derived safety metrics, continuous anticipatory costs, noisy
  human-state estimates, three manipulation tasks, and a
  deployment-faithful evaluation harness (Chapter~\ref{ch:benchmark-env}).
\item \textbf{A two-axis safety measurement protocol.} Proximity
  measures exposure, but a frozen-robot sweep shows that
  $\approx$42\,\% of close proximity is caused by the human approaching
  the robot. We therefore report both proximity exposure and the
  robot-controllable velocity-adaptive \ssm{} axis
  (Section~\ref{sec:setup:metrics}).
\item \textbf{A regime map for choosing the safety mechanism.} The
  key variable is the co-location pattern: whether the human and robot
  are close almost continuously or only in bursts. Persistent
  co-location requires proactive policy learning. Intermittent
  co-location allows a runtime critic to find safe windows in which the
  robot can keep moving at full speed
  (Section~\ref{sec:results:boundary}).
\item \textbf{Mechanism-level lessons.} Learned vetoes freeze, geometric
  dodges flee, and neither gracefully reduces proximity under persistent
  co-location. A binary veto also breaks the action coherence of the
  chunked CQN-AS policy, raising max velocity from $2.46$ to
  $6.03$\,m/s. Continuous speed-scaling preserves action direction, so
  the successful runtime design is: \emph{let the critic decide when to
  intervene, but slow smoothly rather than veto}
  (Sections~\ref{sec:results:reactive-taxonomy} and
  \ref{sec:results:r2-veto}).
\item \textbf{Deployable recipes.} On \texttt{saucepan\_to\_hob}, a
  fixed-multiplier constrained policy reduces proximity by $22.8\,\%$
  at $0.76$ success. On \texttt{dishwasher\_close} and
  \texttt{drawers\_open\_all}, critic-gated speed-scaling reduces
  \ssm{}-velocity violations by $50\,\%$ and $22\,\%$ respectively,
  with moderate success cost. WCSAC is reimplemented as an external
  safe-RL reference (Chapter~\ref{ch:eval}).
\item \textbf{A reproducible value-based constrained-RL formulation.}
  Because CQN-AS is critic-only, the Lagrangian must be written in
  terms of $Q_r-\lambda Q_c$ rather than an actor update. The report
  also proves a C51 support-bound condition showing when dense shaping
  is safe to add to a distributional critic
  (Section~\ref{sec:method:lagrangian}).
\end{enumerate}
...
\section{Ablation Map: What Each Component Proves}
\label{sec:results:ablation-map}

The evaluation is deliberately ablation-led. Each major component of
the Hybrid Safety Critic is removed, replaced, or retuned to test
whether it actually contributes to either proximity exposure or
SSM-velocity safety. The main ablations are summarised here before the
regime-specific results:
\begin{itemize}[topsep=2pt,itemsep=1pt]
\item \textbf{Human-state channel without a safety gradient} (E1.1):
  perception alone does not produce safety; the policy uses oracle
  human state for task completion, not distance keeping.
\item \textbf{Cost form and budget} (E3.1--E3.3): continuous vs.
  binary cost is secondary; the binding question is whether the budget
  is feasible in the task's co-location regime.
\item \textbf{Multiplier update} (E3.2): PID auto-tuning is
  seed-unstable at the feasibility boundary, motivating fixed
  $\lambda$ on saucepan.
\item \textbf{Runtime override form} (E2.4--E2.6): a binary veto
  breaks chunked-policy coherence, unconditional scaling is safe but
  costly, and critic-gated scaling recovers throughput only where safe
  windows exist.
\item \textbf{Gate critic and label design} (E2.6 ablations):
  $R$ is the dominant dial, looser anticipatory labels do not improve
  the frontier, gating a Lagrangian policy does not dominate gating the
  baseline, and DAgger-style re-collection on filtered rollouts
  under-gates.
\end{itemize}
This map is the evidence that the final architecture is not an
accidental combination of parts: each retained component survives a
specific failure mode of a simpler alternative.
...
latter. This observation motivates the
...
pre-registered E4.4 rule failed exactly where the map predicts
(gating $\approx$ unconditional under persistent
...
($61.5\,\%$ vs.\ $19$--$27\,\%$), the pre-registered E4.4 rule failed
exactly where the map predicts, and the
...
was pre-registered before E4.4 ran (the saucepan failure case is a
...
without a working mechanism: the scalar expected-cost Lagrangian either
does not affect the policy or collapses the task there (E3.3), and the
gate, by design, does not reduce exposure. The open lever is a
differently-shaped training signal for
...
should be used. This work contributes three linked pieces: \sbigym{},
a safety-aware extension of the \bigym{}~\cite{chernyadev2024bigym}
benchmark; a measurement showing that $\approx$42\,\% of close
proximity is human-driven and therefore outside robot control; and the
regime map, an empirical rule for matching the safety mechanism to the
human--robot co-location pattern.
...
the robot gets \emph{faster}, not safer; the from-scratch
WCSAC~\cite{yang2021wcsac} reference corroborates the trainability
difficulty. A binary veto breaks chunked-policy coherence (max velocity
$2.46 \to 6.03$\,m/s), and the working mechanism is a learned critic
\emph{gating} an \iso{} speed-scaling backstop: $-50\,\%$
\ssm{}-violation at $-0.10$ success on dishwasher, $-22\,\%$ at
$-0.09$ (or $-10\,\%$ at $-0.02$) on drawers, with the gate threshold
as a deployable dial.
