...
many real safety incidents will originate, and is a deployment
challenge safe-RL benchmarks rarely model. Most expose ground-truth
...
the policy headroom to react. The laboratory behind the unified
\ssm{}/\pfl{} treatment has since carried the combined pre- and
post-contact scheme onto a humanoid upper
body~\cite{rozlivek2025harmonious}.
...
gap}~\cite{tobin2017domainrand,zhao2020sim2real,muratore2022randomized},
that is, the
...
failures. Newer world-grounded estimators
(WHAM~\cite{shin2024wham}, TRAM~\cite{wang2024tram}) are more
accurate but run offline on GPUs, so the noise model stays
calibrated to the on-robot CPU estimator.
...
RCareWorld~\cite{ye2022rcareworld}             & \xmark & \cmark (care)    & \xmark & \xmark & \cmark \\
...
(Assistive Gym~\cite{erickson2020assistivegym},
RCareWorld~\cite{ye2022rcareworld}) to social navigation
...
randomisation~\cite{tobin2017domainrand,zhao2020sim2real,muratore2022randomized}
...
distribution-free, more data-hungry). Ready instruments exist:
off-policy trust-region CVaR constraints~\cite{kim2022offtrc} fit
this replay-based critic-only pipeline most directly, with
distributional multi-constraint and spectral-risk
methods~\cite{kim2023sdac,kim2024srcpo} as on-policy alternatives.
The cost critic is already
...
filter~\cite{bansal2017hamilton,hsu2024safetyfilter} would add provable
...
work that measures that combination. Two smaller items complete this thread:
...
difficulty, as does the PID-Lagrangian failure reported beside a
simulated human at 6 DoF~\cite{thumm2023interventions}. A binary veto breaks chunked-policy coherence (max velocity
