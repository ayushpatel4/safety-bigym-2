...
**P3.1 status (2026-05-20):** `LagrangianCQNASAgent` + λ-PID + dual-Q + `Q_c` written; 10 local unit tests pass (the agent test gates on `tensordict`, GPU-box only). **Remaining:** (1) D3b-validation must pass first — don't trust constrained-RL training until the demo+workspace base is non-degenerate; (2) run the P3.1e 2000-frame GPU smoke (`agent=cqn_as_lagrangian …`, command in Workstream P3.1); (3) commit the branch (ask the user; never `main`). Then hand E3.* sweeps to the human.
