...
learning gradient. Three cells on `reach_target_single`, one per
`bodyslam` mode.

Decision rule (off → mode SSM-violation-rate reduction, averaged across
the 5 disruption types):
  - ≥ +20% reduction → channel + reward together work; greenlight Phase 3.
  - ≈ 0           → channel doesn't help even with reward; pivot to
                    Phase 2 (filter) and/or alternative reward shaping.
  - off wins      → reward shaping alone suffices; retire the channel.

Usage:
    python scripts/phase1_reward_pilot.py --train  # 3 train commands
    python scripts/phase1_reward_pilot.py --eval   # 15 eval commands
...
# Fill after training. Peak-by-W&B-curve snapshot per mode.
SNAPSHOTS: dict[str, str | None] = {m: None for m in BODYSLAM_MODES}
...
def _eval_cmd(
    mode: str, disruption: str, snapshot: Path,
...
def _print_train(seed: int) -> int:
    print(f"# Phase-1.4 reward pilot — 3 train cells on {TASK}")
...
def _print_eval(seed: int, num_eval_episodes: int) -> int:
    print(f"# Phase-1.4 reward pilot — eval ({num_eval_episodes} eps each)")
    missing: list[str] = []
    for mode in BODYSLAM_MODES:
        snap = _resolved_snapshot(mode)
        if snap is None:
            missing.append(mode)
            print(f"# SKIP bs={mode}: no snapshot (rel={SNAPSHOTS.get(mode)!r})")
            continue
        print(f"# --- bs={mode}  ({snap.relative_to(REPO_ROOT)}) ---")
        for disruption in DISRUPTIONS:
            cmd = _eval_cmd(
                mode, disruption, snap,
...
def _run_grid(seed: int, num_eval_episodes: int) -> int:
    ...
    missing = [m for m in BODYSLAM_MODES if _resolved_snapshot(m) is None]
    if missing:
        print(f"# missing snapshots for modes {missing}; run --train first.")
        return 2

    base_env = os.environ.copy()
    results: dict = {}
    n_cells = len(BODYSLAM_MODES) * len(DISRUPTIONS)
    cell_idx = 0
    for mode in BODYSLAM_MODES:
        ...
        for disruption in DISRUPTIONS:
            cell_idx += 1
            print(f"\n# [{cell_idx}/{n_cells}] bs={mode} / {disruption}")
            ...
            try:
                with open(tmp_out) as f:
                    results[mode][disruption] = json.load(f)
    ...
    print("\n" + "=" * 88)
    print(f"PHASE-1.4 REWARD PILOT — {TASK}  "
          f"(avg over {len(DISRUPTIONS)} disruptions)")
    print("=" * 88)
    print(f"{'mode':<8} {'ssm_viol':>10} {'pfl_viol':>10} {'success':>9} "
          f"{'redux':>10} {'criterion':>10}")
    print("-" * 88)
    ...
    for mode in BODYSLAM_MODES:
        ...
        if mode == "off" or off is None or off["ssm"] == 0:
            ...
            if r_pct >= 0.20:
                any_pass = True
        print(f"{mode:<8} {r['ssm']:>10.3f} {r['pfl']:>10.3f} "
              f"{r['success']:>9.2f} {redux:>10} {crit:>10}")
    print("-" * 88)
...
def main() -> int:
    ...
    if args.smoke:
        return _smoke(args.bodyslam, args.seed)
    if args.run:
