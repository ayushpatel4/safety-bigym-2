...
\title{A Hybrid Safety Critic for\\
       ISO~15066-Referenced Humanoid Manipulation\\
       Under Human Co-Working}
\author{Ayush Patel \\
...
industrial arms, and speed-and-separation interlocks all have clear
models and certifiable control logic. Learned humanoid manipulation is
different. A 76-degree-of-freedom robot trained from demonstrations can
now share a workspace with a moving person, but its policy optimises
task reward rather than human distance, speed-and-separation margins,
or contact force. This project studies that greenfield intersection of
state-of-the-art robot manipulation and collaborative-robot safety.

I introduce \sbigym{}, a safety-aware extension of \bigym{} with a
Unitree~G1 humanoid coworker, calibrated mock-\bodyslampp{} perception
noise, \iso{}-derived safety metrics, continuous cost signals, and a
reproducible snapshot-evaluation harness. On top of the
demonstration-driven \cqnas{} backbone, I implement a value-based
Lagrangian constrained-RL policy and runtime safety filters: learned
SVF vetoes, speed-scaling backstops, and a critic-gated speed scaler.
This is technically non-trivial because \cqnas{} is critic-only, so the
standard actor-critic Lagrangian must be re-derived in value-based
form, and dense safety costs must fit inside the C51 value support.

The result is not that one method wins everywhere, but a regime map.
Proximity measures exposure, yet a controlled sweep shows that roughly
42\,\% of close proximity is human-driven and remains even when the
robot is frozen. The robot-controllable axis is therefore
velocity-adaptive \ssm{}. Under persistent co-location
(\texttt{saucepan\_to\_hob}), only anticipatory policy learning reduces
proximity gracefully: fixed-$\lambda$ constrained RL cuts proximity by
22.8\,\% (0.296 to 0.228) at 76\,\% success, while reactive filters
freeze or flee. Under intermittent co-location
(\texttt{dishwasher\_close}, \texttt{drawers\_open\_all}), constrained
RL is inert or task-fatal, but critic-gated speed scaling reduces
\ssm{}-velocity violations by 50\,\% and 22\,\% respectively with
moderate success cost. The Hybrid Safety Critic is thus a two-arm
architecture: anticipatory policy learning where co-location is
persistent, and critic-gated runtime speed control where safe windows
exist.
\end{abstract}
...
is comparatively mature: industrial arms use cages and light curtains,
and driver-assistance functions such as adaptive cruise control and
lane keeping have shipped at scale under established functional-safety
practice. Those examples depend on assumptions that do not hold for a
learned humanoid manipulator. A cage assumes the person and the machine
do not share space. Driver assistance assumes a low-dimensional plant,
a road vehicle, whose controller can be designed and certified in
closed form. A 76-degree-of-freedom manipulation policy learned from
demonstrations satisfies neither assumption. It works in the same
volume a person reaches into, and its behaviour is a neural network,
not a hand-derived control law. Safety for this class of system is
therefore a greenfield problem, and closing it is what this project
targets.

The policies on these platforms are also largely \emph{unaware} of the
standard that should govern their safety. \iso{}~\cite{iso15066}, now
integrated into ISO 10218:2025~\cite{iso10218}, defines two normative
collaboration mechanisms: Speed and Separation Monitoring (\ssm{}),
which sets a velocity-dependent distance bound, and Power and Force
Limiting (\pfl{}), which caps contact forces against biomechanical
limits. Standard imitation-learning and reinforcement-learning agents
on \bigym{}~\cite{chernyadev2024bigym} optimise task success against a
sparse reward. They have no human-distance objective, no \ssm{} margin,
and no constraint that distinguishes a successful trajectory from a
dangerous one.
...
The deployment frontier has outrun the research frontier. Manipulation
benchmarks largely ignore safety; safety benchmarks largely ignore
manipulation; safe-RL methods usually choose either a training-time
constraint or a runtime filter, not both; and most assume cleaner human
state than a robot would have at deployment. These gaps meet exactly
where a safe humanoid manipulator must operate. The area is largely
uncharted: there is not yet a benchmark on which to pose the full
question, let alone a validated method that answers it. This project
enters that intersection by closing three concrete gaps in safe-RL
evaluation for human-co-located humanoid manipulation:
...
\item \textbf{The resulting system has not been characterised under
  realistic perception noise.} The mock-\bodyslampp{} observation
  wrapper introduced here fills the specific gap left by existing
  safe-RL manipulation benchmarks: it bundles a calibrated, lagged and
  dropout-aware human-state channel with the safety metrics and policy
  evaluation, rather than assuming perfect simulator state.
\end{itemize}

The difficulty is that these gaps must be closed \emph{together}. None
of the pieces has an off-the-shelf solution at this scale. A
demonstration-driven, critic-only agent has no actor network to attach
a Lagrangian multiplier to, so the constrained objective must be
re-derived in value-based form
(Section~\ref{sec:method:lagrangian:why}). Dense safety costs must be
kept within the distributional critic's value support, or training
saturates (Proposition~\ref{lem:support-bound}). A runtime safety
filter must be trained on the same noisy perception distribution it
will see at deployment, not on simulator ground truth
(Section~\ref{sec:method:filter}). And the whole system must still
complete the task while a humanoid coworker repeatedly enters the
workspace and reaches toward the robot. The contribution is the
working composition of these parts, the benchmark that makes the
composition measurable, and an honest account of where it helps and
where it does not.
...
\item[RQ1] How should safety be measured when a moving coworker can
  create proximity independently of the robot, and which parts of the
  safety signal are robot-controllable?
\item[RQ2] Under persistent co-location, can a value-based
  Lagrangian constrained-RL policy learn anticipatory avoidance that
  reactive runtime filters cannot provide?
\item[RQ3] Under intermittent co-location, can a learned safety critic
  gate an \iso{} speed-scaling backstop so that the robot improves the
  velocity-adaptive \ssm{} axis without collapsing task success?
\item[RQ4] What boundary condition determines whether constrained
  policy learning or critic-gated runtime filtering carries safety,
  and how does the system degrade outside that regime?
\end{description}
...
This report studies \iso{}-referenced safety for a learned,
high-degree-of-freedom manipulator under human co-presence. Because the
existing literature does not provide either a benchmark for this
setting or a validated method at this scale, the contributions are
deliberately both infrastructural and methodological:

\begin{enumerate}[label=\textbf{C\arabic*.}, leftmargin=2em]
\item \textbf{\sbigym{}, a deployment-faithful benchmark for
human-coworker manipulation.} \sbigym{} adds a Unitree G1 coworker, a
co-working disruption distribution, three \iso{}-derived metric
flavours, continuous \ssm{}/\pfl{} cost signals, calibrated
mock-\bodyslampp{} perception noise, and a snapshot-evaluation harness
(Sections~\ref{sec:bench:metrics:flavours}--\ref{sec:bench:harness}).
The harness exposed four collection-versus-deployment mismatches that
had silently invalidated an earlier filter result; fixing those
mismatches is a methodological contribution.
\item \textbf{A two-axis safety protocol.} Proximity measures exposure,
but a prior controlled sweep shows that roughly $42\,\%$ of it is
human-driven even when the robot is frozen. The report therefore
evaluates both the proximity/exposure axis and the robot-controllable
velocity-adaptive \ssm{} axis, instead of collapsing safety into a
single number.
\item \textbf{The Hybrid Safety Critic architecture.} The architecture
combines a value-based Lagrangian constrained policy with runtime
safety critics and speed-scaling backstops. Because \cqnas{} is
critic-only, the Lagrangian objective must be re-derived in
value-based form, and dense safety shaping requires a C51 value-support
invariant (Proposition~\ref{lem:support-bound}).
\item \textbf{A co-location regime map.} Under persistent co-location
(\texttt{saucepan\_to\_hob}), fixed-$\lambda$ constrained RL reduces
proximity by $22.8\,\%$ at $0.76$ success, while reactive filters
freeze, flee, or become equivalent to unconditional scaling. Under
intermittent co-location (\texttt{dishwasher\_close},
\texttt{drawers\_open\_all}), constrained RL is inert or task-fatal,
but critic-gated speed scaling reduces velocity-adaptive \ssm{}
violations by $50\,\%$ and $22\,\%$ with moderate success cost.
\item \textbf{Mechanism-level lessons for safe robot learning.} A
binary SVF veto breaks action coherence in chunked policies; continuous
speed scaling preserves action direction; PID auto-tuning of
$\lambda$ is seed-unstable at feasibility-boundary budgets; and the
gate critic should be trained on unfiltered rollouts rather than on
its own filtered distribution.
\end{enumerate}
...
the experimental setup and the results, organised around the two-axis
protocol and the persistent/intermittent co-location regimes, with the
regime map (contribution~C4) as the headline. Chapter~\ref{ch:disc} interrogates
...
fixed support is crucial for the safety extension: a shaped reward
...
This problem is already recognised in the safety-filter literature,
not merely an artefact of this system. Recent latent safety
filters~\cite{nakamura2025latent} observe
...
proactive avoidance can---which remains unanswered in the prior
high-DOF manipulation and \iso{}-referenced safe-RL literature reviewed
above.
...
critic on Safety-Gym, much lower-DOF than our setting. A
\sbigym{} WCSAC implementation is underway as the natural external
baseline, but no validated WCSAC number is included in the present
results (Section~\ref{sec:results:wcsac}). CBF-RL~\cite{cbfrl2024} and
...
logging. This layer is the integration point that makes the rest of
the pipeline body-agnostic: swapping the G1
...
\textbf{All quantitative compliance claims in this report are therefore
qualified as \ssm{}/proximity-only, not full \iso{} \pfl{} compliance}
...
\textbf{The engineering lesson generalises beyond \sbigym{}.} The same
check applies to any distributional critic trained with dense shaping:
the discounted shaping return must fit inside the critic's value
support.
...
is the central tool for reproducible benchmark consumption.
...
Those CSV rows are the canonical source for every numerical entry in
...
comparison reported here is the prerequisite for that follow-up.
...
baseline on Safety-Gym, is being implemented as an external comparison
on the same task and disruption distribution. Because the
reimplementation is not yet validated, no WCSAC number is included in
the present results (Sections~\ref{sec:results:wcsac},
...
\subsection{Two Safety Axes: Exposure and Robot-Controllable Velocity}
\label{sec:setup:metrics:primary}

The report uses a two-axis safety protocol rather than a single
headline metric. The first axis is \emph{proximity exposure}:
\texttt{ep\_proximity\_violation\_rate}, the fraction of env-steps in
which $\min$-separation $< \tau_{\mathrm{prox}} = 0.3$\,m
(Section~\ref{sec:bench:proximity-threshold}). This is the natural
axis for policy-level avoidance, but it is not fully robot-controllable:
under the \coworker{} disruption a human can walk into the robot's
workspace even when the robot is stationary.

The second axis is \emph{velocity-adaptive \ssm{}}, reported as
\texttt{ep\_ssm\_violation\_actual\_rate}: whether the robot is moving
slowly enough, at the realised human--robot separation, to satisfy the
\iso{} speed-and-separation margin. This is the primary
robot-controllable axis for runtime filters, because a filter cannot
move the human away but can slow the robot down. Both axes are reported
throughout: the policy's main win is on proximity exposure, while the
critic-gated runtime filter's main win is on velocity-adaptive \ssm{}.
Aggregation uses bootstrap 95\,\% confidence intervals over the
episode set stated in each table.
...
\iso{}~\ssm{} formulation) alongside proximity. The
...
\item $\cvar_{0.95}$ of \texttt{ep\_cost\_integral} (the tail metric
  that matches the cost signal the policy was trained on).
...
E3.7   & External baseline (WCSAC): implementation underway, no validated result yet
...
\section{R0: Evaluation Protocol and Two Safety Axes}
\label{sec:results:baseline}
\label{sec:results:two-axes}

The results are organised by the safety axis and by the
human--robot co-location regime. This is the central methodological
move in the evaluation. Proximity remains important: it measures how
often the robot and coworker occupy the same physical space, and it is
the natural axis for learned anticipatory avoidance. But proximity is
not fully controlled by the robot. A coworker can approach a stationary
robot, creating close proximity even when the robot takes the safest
possible action of not moving.

\begin{figure}[h]
\centering
\includegraphics[width=0.82\textwidth]{fig4_exogenous_proximity.png}
\caption{A prior saucepan SVF sweep decomposes proximity into a
robot-controllable component and a human-driven floor. Increasing the
runtime filter intervention rate to 100\,\% freezes the robot, yet
time-in-proximity falls by only $\sim$58\,\%. The remaining
$\sim$42\,\% is the coworker approaching a stationary robot. This
motivates the two-axis protocol: proximity is retained as an exposure
metric and policy-avoidance axis, while velocity-adaptive \ssm{} is the
primary robot-controllable axis for runtime filters.}
\label{fig:exogenous-proximity}
\end{figure}

This decomposition is not a post-hoc metric switch. First, it comes
from a controlled sweep of the saucepan filter before the critic-gated
speed-scaling result. Second, every main table reports both proximity
and velocity-adaptive \ssm{}. Third, the two-axis framing cuts both
ways: the constrained policy's strongest result is on proximity, while
the runtime filter's strongest result is on the robot-controllable
\ssm{}-velocity axis.

\begin{table}[h]
\centering
\caption{Axis ownership used throughout the results. Proximity
measures exposure and is partly human-driven; velocity-adaptive
\ssm{} is the axis directly controllable by runtime action scaling.}
\label{tab:axis-ownership}
\small
\begin{tabular}{lll}
\toprule
Safety axis & Reported metric & Mechanism expected to help \\
\midrule
Proximity exposure & \texttt{ep\_proximity\_violation\_rate} at $\tau=0.3$\,m
  & Anticipatory policy learning \\
Robot-controllable SSM velocity & \texttt{ep\_ssm\_violation\_actual\_rate}
  & Runtime speed scaling / gating \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Observation Diagnostic}
\label{sec:results:obs}
\label{sec:results:obs-bc}
\label{sec:results:obs-rl}

The Phase~1 observation ablation explains why perception alone is not a
safety mechanism. Under pure behaviour cloning (ACT, four tasks,
\texttt{off}/\texttt{oracle}/\texttt{noisy}, no penalty), no cell
cleared the pre-registered 20\,\% reduction bar on the legacy
\texttt{ssm\_violation} metric. The most informative case was
\texttt{saucepan\_to\_hob}: oracle human state improved task success
($0.22\rightarrow0.58$) while worsening SSM violations
($0.135\rightarrow0.203$). The policy used human state for task
completion, not safety. Under the constrained policy, by contrast, the
same human channel supports avoidance, and that avoidance is robust to
deployment noise: proximity is statistically similar under oracle and
noisy perception ($0.236$ vs.\ $0.198$ on the matched checkpoint).

\section{R1: Persistent Co-location Requires Anticipatory Avoidance}
\label{sec:results:persistent}

The primary persistent-co-location task is
\texttt{saucepan\_to\_hob}. The coworker repeatedly enters the shared
workspace and reaches near the robot's end-effector and task object.
In this regime, the robot and human are close for sustained periods, so
reactive filters fire almost continuously. The result is a clean
separation of roles: constrained policy learning can reduce proximity
exposure, while runtime scaling can reduce robot speed, but achieving
both simultaneously stacks their costs.

\subsection{Constrained Policy Learning on the Proximity Axis}
\label{sec:results:cost-signal}
\label{sec:results:budget}
\label{sec:results:headline-feature-table}

The constrained-RL result is not a generic ``more cost is better''
story. E3.1 shows that cost \emph{form} is secondary: at a tight budget
$d=0.01$, both continuous and binary Lagrangian variants collapse to
0\,\% task success because the budget sits below the task's inherent
cost. E3.2 shows that PID auto-tuning is seed-unstable at the only
non-trivial budget: at $d=0.3$, $\lambda$ converges to
$0.000/0.267/3.855$ across seeds, producing an unconstrained policy, a
graceful avoidance basin, and windup collapse. Pinning
$\lambda=0.1$ removes this ill-conditioned dual loop.

\begin{figure}[h]
\centering
\includegraphics[width=0.82\textwidth]{d0p3_3seed_lambda_regimes.png}
\caption{PID-Lagrangian at the feasibility boundary on
\texttt{saucepan\_to\_hob}. The same budget $d=0.3$ produces
unconstrained, graceful, and collapsed regimes across three seeds,
motivating the fixed-$\lambda$ operating point.}
\label{fig:lambda-regimes}
\end{figure}

At the selected fixed-$\lambda=0.1$ operating point, the policy reduces
proximity exposure from $0.296$ to $0.228$ at $0.76$ task success,
pooled over three trained seeds. The improvement is modest in absolute
terms because the exogenous floor is real, but it captures a large
fraction of the robot-driven proximity that can be reduced at all.

\begin{table}[h]
\centering
\caption{Persistent co-location on \texttt{saucepan\_to\_hob}. The
policy owns the proximity axis, runtime scaling owns the
velocity-adaptive \ssm{} axis, and the policy--filter composition is
the only both-axis configuration but pays the stacked success cost.}
...
\toprule
Configuration & \texttt{succ.} & \texttt{prox.viol} & $\Delta$ prox.\ & \texttt{ssm-act.} & $\Delta$ ssm.\ & Interpretation \\
\midrule
Unconstrained baseline & $0.85$ & $0.296$ & -- & $0.146$ & -- & task-competent, unsafe \\
Fixed-$\lambda$ policy & $0.76$ & $\mathbf{0.228}$ & $\mathbf{-23\,\%}$ & $0.112$ & $-23\,\%$ & proximity-axis win \\
Speed scaling on baseline & $0.53$ & $0.273$ & $-8\,\%$ & $\mathbf{0.048}$ & $\mathbf{-67\,\%}$ & velocity-axis win \\
Policy + speed scaling & $0.44$ & $0.250$ & $-15\,\%$ & $0.065$ & $-55\,\%$ & both axes, stacked cost \\
\bottomrule
\end{tabular}%
}
\end{table}
...
\begin{table}[h]
\centering
\caption{Per-trained-seed breakdown for the selected fixed-$\lambda =
0.1$ policy checkpoints. Each row is benchmarked under the same
\texttt{noisy} \coworker{} deployment condition for 60 episodes.}
\label{tab:e4.1-row3-seeds}
\small
\begin{tabular}{lcccc}
\toprule
Train seed & Selected step & \texttt{succ.} & \texttt{prox.viol} & \texttt{ssm-act.} \\
\midrule
0 & 30{,}546 & $0.750$ & $0.198$ & $0.088$ \\
1 & 8{,}225  & $0.783$ & $0.274$ & $0.153$ \\
2 & 32{,}696 & $0.750$ & $0.213$ & $0.096$ \\
\midrule
Pooled & -- & $0.76$ & $0.228\,[0.19,0.26]$ & $0.112\,[0.09,0.13]$ \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Reactive Filters on the Proximity Axis}
\label{sec:results:filter-pareto}
\label{sec:results:reactive-taxonomy}
\label{sec:results:interaction}

Reactive filters are bounded by the freeze-versus-flee dilemma under
persistent co-location. The learned SVF veto with a zero-velocity
fallback freezes: it reduces neither proximity nor velocity-adaptive
\ssm{} on the baseline, and when applied to the already-avoiding policy
it over-vetoes out of distribution (48\,\% intervention), worsening
proximity and success. Model-based dodges flee: the base-dodge filter
can reduce proximity (0.198 to 0.150 at $d_{\mathrm{target}}=0.35$),
but only by retreating from the task and dropping success from 0.75 to
0.57. End-effector retract behaves similarly. Re-collecting the critic
on the policy's own rollouts does not salvage a graceful veto point.

\begin{figure}[h]
\centering
\includegraphics[width=0.86\textwidth]{filter_taxonomy.png}
\caption{Reactive filters under persistent co-location either freeze or
flee. The proactive policy is the only tested method that reaches a
graceful proximity--success frontier; learned vetoes dwell, and
model-based dodges buy distance by sacrificing the task.}
\label{fig:filter-taxonomy}
\end{figure}

\subsection{The Persistent-Regime Ceiling}
\label{sec:results:joint-coverage}

Runtime speed scaling is still useful, but on the velocity axis. It
reduces \texttt{ep\_ssm\_violation\_actual\_rate} from 0.146 to 0.048
on the baseline, but because the coworker is persistently co-located,
it fires often and costs success. Composing the fixed-$\lambda$ policy
with speed scaling reaches both measured axes at once, but success
falls to 0.44. A later critic-gated speed-scaling sweep on saucepan
confirms the regime interpretation: as the gate threshold $R$ rises,
the point slides from policy-alone toward unconditional scaling rather
than opening a high-success, low-SSM corner.

\begin{table}[h]
\centering
\caption{Critic-gated speed scaling on \texttt{saucepan\_to\_hob}. The
gate does not rescue the persistent-regime C4 cost: it simply dials
between the unfiltered policy and unconditional scaling. This sweep ran
on \texttt{snapshot\_best}; the unconditional control reproduces the
headline filter axes (0.433/0.072 vs.\ 0.44/0.065), so the verdict is
filter-level rather than a replacement for the basin-picked policy
row.}
\label{tab:saucepan-gated}
\small
\begin{tabular}{lcccc}
\toprule
Saucepan configuration & Success & SSM-actual & $\Delta$ SSM & $\Delta$ success \\
\midrule
...
Unconditional speed scale & $0.433$ & $0.072$ & $-48\,\%$ & $-0.29$ \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Robustness and Tail Risk}
\label{sec:results:tail}
\label{sec:results:ood}
\label{sec:exogenous}

The saucepan policy's mean behaviour generalises, but the worst-case
tail remains exogenous. Under the wider, gentler
\texttt{coworker\_eval} disruption, the fixed-$\lambda$ policy still
reduces proximity (0.084 to 0.058, $-31\,\%$), although the success
cost increases because $\lambda=0.1$ is tuned for the harder training
distribution. The closest-approach tail does not move: CVaR$_5$ of
minimum separation remains $\approx0.005$\,m across configurations.
The policy controls proximity frequency and dwell, not the single
human-driven closest lunge.

\section{R2: Intermittent Co-location Favours Critic-Gated Speed Scaling}
\label{sec:results:intermittent}
\label{sec:results:wcsac}

The dishwasher and drawers tasks present a different regime. The human
is still a live G1 coworker under the same \coworker{} disruption, but
co-location is intermittent: there are windows in which the task can
proceed safely at full speed. In this regime, the constrained policy
does not find a robust safety-improving operating point, but a runtime
critic can identify when speed scaling is actually needed.

\subsection{Why the Alternatives Fail}

On \texttt{dishwasher\_close} and \texttt{drawers\_open\_all}, adaptive
Lagrangian budgets above the natural operating cost leave the constraint
inert; budgets below it make $\lambda$ bind and collapse the task. When
the constraint binds, the robot becomes faster and more erratic rather
than carefully slower (mean velocity rises from 0.44 to 0.79 in the
retarget sweep). WCSAC is being implemented as an external reference;
early from-scratch runs corroborate the trainability difficulty
(dishwasher learns partially, drawers does not), so these results are
reported as a limitation of the current training recipe rather than a
universal claim that constrained RL cannot work.

A binary SVF veto also fails. On chunked temporal-ensemble policies, a
zero-velocity veto breaks action-sequence coherence: the robot stops
mid-chunk and then overshoots when the veto lifts. Continuous
speed-scaling avoids this because it scales the action magnitude while
preserving its direction. The correct runtime design is therefore to
gate the speed scaler, not to veto the action.

\subsection{Critic-Gated Speed Scaling}
\label{sec:results:gated-scaling}

Critic-gated speed scaling separates \emph{when} to intervene from
\emph{how} to intervene. The SVF critic flags risky state--action pairs;
when it fires, the filter applies the smooth \iso{} speed-scaling rule.
When it does not fire, the policy executes at full speed. This is the
runtime arm of the Hybrid Safety Critic.

\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{fig1_method_comparison.png}
\caption{Runtime safety methods on the robot-controllable
SSM-velocity axis. The desirable region is high success and low
velocity-adaptive \ssm{} violation. On intermittent co-location tasks,
critic-gated speed scaling reaches the safe-and-on-task corner. The
drawers Lagrangian point is a basin-selected diagnostic, not a stable
deployable budget: the full sweep is inert-or-fatal, and gating the
backstop on the Lagrangian policy behaves like gating it on the
baseline.}
\label{fig:gated-method-comparison}
\end{figure}

\begin{table}[h]
\centering
\caption{Method comparison on intermittent co-location tasks. The
critic-gated scaler is the only evaluated runtime method that reduces
velocity-adaptive \ssm{} substantially while preserving most task
success. Both axes are reported; the headline comparison is on the
robot-controllable \ssm{} axis.}
\label{tab:intermittent-methods}
\small
\setlength{\tabcolsep}{3.5pt}
\resizebox{\textwidth}{!}{%
\begin{tabular}{llcccc}
\toprule
Task & Method & Success & SSM-actual & $\Delta$ SSM & $\Delta$ success \\
\midrule
dishwasher & Baseline & $0.77$ & $0.176$ & -- & -- \\
dishwasher & Lagrangian basin & $0.82$ & $0.184$ & $+5\,\%$ & $+0.05$ \\
dishwasher & SVF veto ($R=2.25$) & $0.63$ & $0.174$ & $-1\,\%$ & $-0.14$ \\
dishwasher & Speed-scale, unconditional & $0.52$ & $0.087$ & $-50\,\%$ & $-0.25$ \\
dishwasher & \textbf{Critic-gated scaling} & $\mathbf{0.67}$ & $\mathbf{0.088}$ & $\mathbf{-50\,\%}$ & $\mathbf{-0.10}$ \\
\midrule
drawers & Baseline & $0.82$ & $0.100$ & -- & -- \\
drawers & Lagrangian basin & $0.77$ & $0.082$ & $-18\,\%$ & $-0.05$ \\
drawers & SVF veto ($R=2.25$) & $0.43$ & $0.130$ & $+30\,\%$ & $-0.39$ \\
drawers & Speed-scale, unconditional & $0.38$ & $0.059$ & $-41\,\%$ & $-0.44$ \\
drawers & \textbf{Critic-gated scaling} & $\mathbf{0.73}$ & $\mathbf{0.078}$ & $\mathbf{-22\,\%}$ & $\mathbf{-0.09}$ \\
\bottomrule
\end{tabular}%
}
\end{table}

The gate threshold $R$ is an interpretable safety--throughput dial. A
low threshold makes the gate selective and near-baseline; a high
threshold approaches unconditional scaling. On dishwasher, the gated
frontier reaches essentially the same SSM floor as unconditional
scaling at much higher success. On drawers, unconditional scaling
retains the lowest SSM floor, but the gated frontier buys most of the
safety improvement at far lower task cost.

\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{fig2_tradeoff_curve.png}
\caption{The gate threshold $R$ dials the safety--success frontier. On
dishwasher, critic-gated scaling matches the unconditional SSM floor at
higher success. On drawers, the unconditional point remains the lowest
SSM value, but gated scaling traces a practical frontier with much
less task collapse.}
\label{fig:gated-tradeoff}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{fig3_reduction_bars.png}
\caption{SSM-violation reduction by method, with success cost
annotated. The critic-gated scaler is the only method in the
intermittent regime that substantially reduces the robot-controllable
SSM-velocity axis without collapsing the task.}
\label{fig:gated-reduction-bars}
\end{figure}

\begin{table}[h]
\centering
\caption{Recommended runtime operating points for intermittent
co-location. The drawers near-free setting is useful when task
throughput is prioritised over maximal SSM reduction.}
\label{tab:gated-operating-points}
\small
\begin{tabular}{llccc}
\toprule
Task & Filter config & Success & SSM-actual & Note \\
\midrule
\texttt{dishwasher\_close} & gated, $R=2.75$, $d_{\mathrm{slow}}=0.5$ & $0.67$ & $0.088$ & $-50\,\%$ SSM \\
\texttt{drawers\_open\_all} & gated, $R=3.0$, $d_{\mathrm{slow}}=0.8$ & $0.73$ & $0.078$ & $-22\,\%$ SSM \\
\texttt{drawers\_open\_all} & gated, $R=2.75$, $d_{\mathrm{slow}}=0.8$ & $0.80$ & $0.090$ & near-free, $-10\,\%$ SSM \\
\bottomrule
\end{tabular}
\end{table}

Ablations support the design. Sweeping $d_{\mathrm{slow}}$ and $R$
shows that $R$ is the dominant dial. Looser, anticipatory critic labels
(0.20 or 0.30\,m) do not improve the frontier: they merely make the
gate less selective. Gating the scaler on the best available
Lagrangian policy behaves like gating on the baseline, confirming that
the intermittent-task Lagrangian did not learn a separate proactive
avoidance strategy. Finally, re-training the critic on the filtered
policy's own rollouts under-gates: once the filtered distribution looks
safe, the critic stops firing. The gate should be trained on unfiltered
policy behaviour.

\section{R3: The Co-location Regime Map}
\label{sec:results:summary}
\label{sec:results:regime-map}

The saucepan and dishwasher/drawers results are not contradictory. They
are two regimes of the same system. Critic-gated speed scaling helps
when co-location is intermittent, because genuinely safe windows exist
for the critic to identify. Under persistent co-location, the gate is
active almost whenever the robot is near the human, so gating
degenerates toward unconditional scaling. In that regime, proximity
requires anticipatory policy motion, and both-axis safety is
purchasable only at stacked success cost.

\begin{figure}[h]
\centering
\includegraphics[width=0.92\textwidth]{fig5_cross_task_boundary.png}
\caption{Boundary condition for the Hybrid Safety Critic. Dishwasher
and drawers bend downward: SSM violations fall while much of task
success is retained. Saucepan slides along the diagonal: safety
improves only as success falls. The same mechanism explains both the
win and the non-win. Critic-gated speed scaling recovers throughput
only when human--robot co-location is intermittent.}
\label{fig:cross-task-boundary}
\end{figure}

\begin{table}[h]
\centering
\caption{Regime map summarising the empirical rule learned from the
three tasks. The method that carries safety depends on the co-location
regime and on which safety axis is being targeted.}
...
\begin{tabular}{llll}
\toprule
Regime & Tasks & What works & What fails \\
\midrule
Persistent co-location &
\texttt{saucepan\_to\_hob} &
Fixed-$\lambda$ policy on proximity; speed scaling on velocity at heavy cost &
Reactive proximity filters; critic-gating (approximately unconditional) \\
Intermittent co-location &
\texttt{dishwasher\_close}, \texttt{drawers\_open\_all} &
Critic-gated speed scaling on SSM-velocity &
Expected-cost Lagrangian budgets; binary vetoes on chunked policies \\
\bottomrule
\end{tabular}%
}
\end{table}

This regime map is the thesis result. It avoids two weaker readings.
The first would be ``proactive constrained RL beats reactive filters'',
which is true only for the proximity axis under persistent co-location.
The second would be ``the gated filter wins'', which is true only for
the SSM-velocity axis under intermittent co-location. The stronger
claim is conditional and predictive: the safety mechanism should be
chosen according to the co-location regime and the axis being targeted.

\section{Summary of Results}
\label{sec:results:old-summary-alias}

\textbf{RQ1 (measurement).} Proximity is an exposure metric with a
human-driven floor; velocity-adaptive \ssm{} is the robot-controllable
runtime-filter axis. Both are reported throughout.

\textbf{RQ2 (persistent co-location).} On \texttt{saucepan\_to\_hob},
fixed-$\lambda$ constrained RL reduces proximity by 22.8\,\% at 0.76
success. Reactive filters cannot gracefully reduce proximity; they
freeze or flee.

\textbf{RQ3 (intermittent co-location).} On
\texttt{dishwasher\_close} and \texttt{drawers\_open\_all},
critic-gated speed scaling is the strongest runtime method on the
SSM-velocity axis, reducing violations by 50\,\% and 22\,\% with
moderate success cost.

\textbf{RQ4 (boundary condition).} Critic-gating recovers throughput
when safe windows exist. Under persistent co-location it degenerates
toward unconditional scaling; under intermittent co-location it opens a
safe-and-on-task operating region.
...
training objective has no incentive for the latter. This observation
motivates the constrained-RL formulation: human-state observation
without a safety reward gradient is at best inert and at worst
counterproductive.
...
\subsection{External Baseline (WCSAC): Implementation Underway}
\label{sec:results:wcsac}

An external WCSAC~\cite{yang2021wcsac} comparison is currently being
implemented as the most direct distributional safe-RL baseline on the
same 76-DOF task and \coworker{} distribution. It is not reported here
because the reimplementation has not yet passed the required validation
against the original Safety-Gym setting and the \sbigym{} training
recipe. The positioning of the hybrid against WCSAC and the four other
safe-RL families it draws on is therefore argued
\emph{structurally}, via the five-axis comparison in
...
\textbf{This is the headline.} The central question is not simply
whether ``the hybrid'' works. It is which mechanism reduces which
safety axis, and whether the mechanisms compose. The answer is a
\emph{division of labour}: the proactive constrained-RL policy reduces
geometric proximity, while a complementary runtime filter controls the
velocity-adaptive \ssm{} axis. Table~\ref{tab:e4.1-feature-incremental}
reports the four configurations that establish this. The learned-veto
variants that motivated the original architecture are analysed
separately in Section~\ref{sec:results:reactive-taxonomy}, and the
both-axis composition in Section~\ref{sec:results:joint-coverage}.

\paragraph{The proactive policy reduces proximity, reproducibly.}
The fixed-$\lambda$ ($\lambda = 0.1$) Lagrangian policy reduces
\texttt{ep\_proximity\_violation\_rate} from $0.296$ to $0.228$, a
$22.8\,\%$ reduction (bootstrap 95\,\% CI $[0.194, 0.264]$), while
preserving $0.76$ task success (CI $[0.70, 0.82]$) over three trained
seeds and 180 evaluation episodes. Each selected seed is below the
baseline, and the velocity-adaptive \ssm{}-actual rate also improves
($0.146 \rightarrow 0.112$, $-23\,\%$). This is the graceful proximity
reduction the reactive filter could not achieve
(Section~\ref{sec:results:filter-pareto}). The absolute gain is bounded
by the $\approx$42\,\% exogenous floor (Section~\ref{sec:exogenous}):
the policy captures roughly $40\,\%$ of the robot-driven proximity
that can be reduced at all.
...
\centering
\caption{Per-trained-seed breakdown for the selected fixed-$\lambda =
0.1$ policy checkpoints in Table~\ref{tab:e4.1-feature-incremental}.
Each row is benchmarked under the same \texttt{noisy}
\coworker{} deployment condition for 60 episodes (3 evaluation seeds
$\times$ 20). All three selected operating points sit below the
unconstrained baseline proximity $0.296$, showing that the pooled
reduction is not a single-seed artefact.}
\label{tab:e4.1-row3-seeds}
\small
\begin{tabular}{lcccc}
\toprule
...
configuration that reduces both measured \ssm{}/proximity axes at once:
geometric
...
\emph{the measured proximity and velocity safety target is reachable
only by composing the proactive policy with a complementary-axis
reactive filter, and that composition makes the safety--throughput
trade explicit}, with a cost dial tunable through
...
baseline. This is the filter's proper axis, complementing the policy's
proximity axis.}
...
compromise, not a cost-free safety layer.}
...
\textbf{RQ1 (obs channel).} \emph{Conditionally useful, and robust to
perception noise.} Under behaviour cloning (E1.1), the human-state
channel is inert or counterproductive. Under the constrained policy it
supports anticipatory avoidance, and that avoidance survives noisy,
lagged perception (E3.6: proximity $0.236$ oracle vs.\ $0.198$ noisy).
A ``perception-off'' mode would change the trained policy's input size,
so the valid comparison here is oracle vs.\ noisy; the no-human-channel
case is the separate E1.1 BC ablation.

\textbf{RQ2 (constrained RL).} \emph{Yes, but only with a stable
operating point.} The cost \emph{form} is not the decisive variable
(E3.1): at tight budgets both continuous and binary forms collapse
because the budget is below the task's inherent cost. The budget is the
binding variable (E3.2). Budgets $d \le 0.2$ are infeasible, while
$d = 0.3$ lies on the feasibility boundary and makes PID
auto-tuning seed-unstable. Pinning $\lambda = 0.1$ gives the stable
operating point: $-22.8\,\%$ proximity at $0.76$ success across three
seeds.

\textbf{RQ3 (composition).} \emph{Division of labour, not free
complementarity.} The learned veto is a mild velocity backstop on the
baseline but is \emph{counterproductive} on the already-avoiding policy
(E4.1, E4.3), because the baseline-calibrated critic is out of
distribution. Across the reactive-filter taxonomy (veto, base-dodge,
EE-retract, speed-scaling), no reactive mechanism reaches the policy's
graceful frontier. The policy owns proximity; speed-scaling owns
velocity; their composition improves both measured axes, but success
falls from $0.85$ to $0.44$.
...
\textbf{RQ1: How should safety be measured when proximity is partly
human-driven?} The answer is a two-axis protocol. Proximity remains the
right exposure metric and the right axis for policy-level avoidance,
but it is not fully controlled by the robot: freezing the robot still
leaves roughly 42\,\% of the close-proximity time. Runtime filters are
therefore evaluated primarily on the robot-controllable
velocity-adaptive \ssm{} axis, while proximity is retained in every
table to prevent hiding exposure.

\textbf{RQ2: What works under persistent co-location?} On
\texttt{saucepan\_to\_hob}, where the coworker remains near the robot
for sustained periods, reactive filters cannot gracefully reduce
proximity. Learned vetoes freeze and dwell; model-based dodges flee
from the task; critic-gated scaling behaves like unconditional scaling
because the gate is active almost continuously. The only graceful
proximity reduction comes from anticipatory policy learning:
fixed-$\lambda$ constrained RL reduces proximity by 22.8\,\% at 0.76
success.

\textbf{RQ3: What works under intermittent co-location?} On
\texttt{dishwasher\_close} and \texttt{drawers\_open\_all}, the
co-location pattern changes. There are safe windows in which the robot
can proceed at full speed, and a learned critic can identify them. In
this regime, expected-cost Lagrangian training is inert or task-fatal,
and binary SVF vetoes break chunked-policy action coherence. Critic-
gated speed scaling is the useful runtime arm: it reduces
velocity-adaptive \ssm{} violations by 50\,\% on dishwasher and
22\,\% on drawers at moderate success cost.

\textbf{RQ4: What is the boundary condition?} The boundary is
co-location persistence. Critic-gating recovers task throughput when
co-location is intermittent, because the critic can pass through
genuinely safe actions. When co-location is persistent, the gate is
nearly always active and the method slides toward unconditional speed
scaling. The Hybrid Safety Critic is therefore not one mechanism with a
single global operating point; it is a two-arm architecture whose
active safety mechanism depends on the regime.

\section{Threats to Validity}
\label{sec:disc:threats}

\subsection{Metric Reframing and Goalpost Risk}
\label{sec:disc:metric-reframe}

The move from a single proximity headline to a two-axis protocol is the
main interpretive risk in the report. It is defensible for three
reasons. First, the exogenous-proximity decomposition comes from a
controlled saucepan sweep, not from the later critic-gated speed-scaling
result. Second, both axes are reported for every headline
configuration. Third, the reframing does not flatter a single method:
the policy win is on proximity, while the runtime-filter win is on
velocity-adaptive \ssm{}. The result is axis ownership, not metric
abandonment.

\subsection{The \pfl{} Contact-Detection Limitation}
\label{sec:disc:pfl-bug}

\textbf{The compliance claim is \ssm{}/proximity-only.} The \pfl{} side
of \iso{} requires meaningful human--robot contact forces, but the
current \bigym{}/mojo runtime attachment produces
\texttt{data.ncon = 0} for those pairs despite geometric overlap. The
\pfl{} schema, limits table, cost signal and labeller are wired, but
$c^{\pfl}_t \equiv 0$ throughout the experiments. Contact-force
metrics are therefore omitted rather than reported as zero.

\subsection{Regime Map Scope}
\label{sec:disc:arch-gen}

The regime map is validated on three manipulation tasks, not on every
possible human-coworker interaction. It is strongest as a mechanism
claim: intermittent co-location gives the critic safe windows to pass
through, while persistent co-location keeps the gate active. The
natural next test is a controlled persistence-dial experiment, holding
task identity fixed while varying the coworker's dwell pattern. That
would turn the three-task boundary observation into a causal test of
co-location persistence.

\subsection{Training Recipe and External Baselines}
\label{sec:disc:wcsac-honest}

The intermittent-task constrained-RL failures are claims about the
current expected-cost Lagrangian recipe, not a proof that constrained
RL is impossible. Sparse-reward task trainability is a real confound:
when $\lambda$ binds, the policy often becomes fast and erratic rather
than cautiously slow. WCSAC~\cite{yang2021wcsac} is the correct
external safe-RL reference and is being implemented; preliminary runs
are used only as corroborating evidence of trainability difficulty, not
as a final comparative benchmark.

\subsection{Curriculum Dependence}
\label{sec:disc:curriculum}

The results presuppose task-competent CQN-AS curriculum policies.
Direct training on the full \coworker{} distribution collapses, which
is unsurprising for long-horizon sparse-reward manipulation but still
limits deployment generality. The critic-gated runtime method is
attractive partly because it can improve a trained policy without
retraining it, but its gate must still be calibrated to that policy and
regime.

\subsection{Sim-to-Real: Perception, Dynamics and Contact}
\label{sec:disc:sim2real}

The mock-\bodyslampp{} model approximates reported BodySLAM++
performance but is not a real perception stack. The Unitree H1 dynamics
in MuJoCo are not independently validated against the physical robot,
and even a fixed \pfl{} contact pipeline would need real collision-force
validation before deployment. The report therefore claims a
deployment-relevant simulation benchmark and mechanism study, not
real-world certification.

\section{Broader Impact}
\label{sec:disc:broader}

The main responsible-deployment lesson is that runtime safety cannot be
presented as a free add-on. In intermittent co-location, critic-gated
speed scaling gives a useful safety--throughput dial. In persistent
co-location, the same mechanism approaches unconditional slowing and
task success falls. A deployment would need to expose that dial and the
regime assumptions behind it, rather than promising generic ``safe
robot learning.'' The same caveat applies to vulnerable domestic users:
\iso{} targets industrial collaboration with trained adults; children,
elderly users or people with reduced reaction time require stricter
evidence than this project provides.

\subsection{Reproducibility as Scientific Discipline}
\label{sec:disc:repro}

The snapshot harness, seeded evaluation, bootstrap confidence
intervals, public code, and archived model checkpoints are part of the
scientific contribution. Several early positive filter results were
invalidated by collection-versus-deployment mismatches; the final
report treats those failures as evidence for why deployment-faithful
...
is feasible. This does not invalidate the result (curriculum learning
is the standard solution to long-horizon sparse-reward
...
establish the same limit empirically in the specific high-DOF
manipulation and \iso{}-referenced setting studied here, and to show
what the policy can do that the filter cannot.
...
\coworker{} distribution, is underway but not yet validated
(Section~\ref{sec:results:wcsac}). We flag this as a genuine limitation
of the present empirical evaluation rather than presenting a partial or
unvalidated number, and the architecture is therefore positioned against
WCSAC and the other safe-RL families \emph{structurally} via
Table~\ref{tab:related-work-comparison}. The soundness check remains
essential: the WCSAC reimplementation should first be validated
against its reported Safety-Gym numbers \emph{on Safety-Gym} before any
\sbigym{} comparison is reported, so that underperformance at this scale
can be cleanly attributed to scale and disruption distribution rather
than to reimplementation error. Conflating ``WCSAC is ineffective at
this scale'' with ``our reimplementation has bugs'' would not be
publishable; deferring the comparison until it can be made cleanly is
the correct call given the current evidence.
...
\section{Persistence-Dial Experiment}
\label{sec:fw:persistence-dial}

The strongest next experiment is to vary co-location persistence while
holding task identity as fixed as possible. The regime map predicts
that critic-gated speed scaling should bend downward in the
success--SSM plane when safe windows exist, and should collapse toward
unconditional scaling when the coworker remains persistently near the
robot. A practical test is to modify the dishwasher coworker schedule
so that it dwells near the robot for longer, then rerun the same
critic-gated $R$ sweep. If the frontier flattens into the saucepan
diagonal, the boundary condition becomes a causal result rather than a
three-task observational pattern.

\section{Learning Proactive Avoidance Beyond Expected-Cost Lagrangians}
\label{sec:fw:proactive}

The current fixed-$\lambda$ policy finds a useful saucepan avoidance
basin, but the expected-cost Lagrangian does not produce robust
proactive avoidance on the intermittent tasks. A more targeted training
signal may be needed: for example, a potential-based separation reward,
a task-aware human-occupancy predictor, or a curriculum that explicitly
rewards clearing the shared workspace before the coworker arrives. This
is the likely route beyond the exogenous proximity ceiling, but it
must be tested carefully because the task geometry often requires the
robot to occupy the very appliance region the human traverses.
...
\noindent
This project started from a greenfield problem: state-of-the-art
humanoid manipulation policies can now operate beside people, but they
are not trained or evaluated against the safety quantities that matter
for human co-working. Classical safety mechanisms such as lane
assistance, cruise control and industrial speed-and-separation
interlocks rely on hand-modelled systems. A 76-DOF demonstration-driven
humanoid policy beside a moving coworker is a different setting. The
first contribution of this work is therefore \sbigym{}, a benchmark and
evaluation harness that makes \iso{}-referenced proximity and
velocity-adaptive \ssm{} safety measurable under realistic noisy human
state.

\noindent
The empirical contribution is the co-location regime map. Safety does
not reduce to choosing the most sophisticated mechanism. It depends on
which part of safety is being controlled and whether the human is
persistently or intermittently co-located with the robot. Proximity is
an exposure axis, and roughly 42\,\% of it is human-driven in the
studied scenes. Under persistent co-location
(\texttt{saucepan\_to\_hob}), reactive filters cannot gracefully reduce
that exposure: they freeze and dwell or flee and abandon the task. The
fixed-$\lambda$ constrained policy is the useful arm there, reducing
proximity by 22.8\,\% at 0.76 success. Under intermittent co-location
(\texttt{dishwasher\_close} and \texttt{drawers\_open\_all}), the
useful arm is different: a learned safety critic gates an
\iso{} speed-scaling backstop, reducing velocity-adaptive \ssm{}
violations by 50\,\% and 22\,\% with moderate success cost. On
saucepan the same gate does not rescue throughput, because the gate is
active almost continuously. That failure is not an anomaly; it is the
boundary condition.

\noindent
The Hybrid Safety Critic should therefore be read as a two-arm
architecture plus a rule for when each arm carries safety. Anticipatory
policy learning is needed when proximity itself must be reduced under
persistent co-location. Critic-gated runtime speed control is the
practical deployable backstop when safe windows exist and the
robot-controllable SSM-velocity axis is the target. The project also
produces several methodological lessons: deployment-faithful collection
matters, binary vetoes can break chunked policies, the gate critic
should be trained on unfiltered rollouts, and dense shaping must fit
inside the C51 value support. These are the contributions that should
transfer beyond \sbigym{}.

\noindent
The result is intentionally qualified. The \pfl{} force loop is not
evaluated because contact detection is currently broken; the benchmark
is simulated; and the regime map is validated on three tasks rather
than every possible human--robot interaction. But that qualified result
is still the useful one: it tells a future designer not simply to add a
filter or tune a Lagrangian, but to ask first which safety axis is
controllable and what co-location regime the robot is operating in.
...
closing that measured \ssm{}/proximity gap: \sbigym{}, a safety-aware extension of the
...
composition each can and cannot contribute to the measured proximity
and velocity-adaptive \ssm{} axes.
...
the accounting of the exogenous floor, are as important as the headline
numbers.
