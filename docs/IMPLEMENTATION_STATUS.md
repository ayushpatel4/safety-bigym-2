...
- [x] **Stage 2 (unconstrained baseline) — COMPLETED (2026-05-22).** Full `coworker_train` disruption. `episode_reward` started at **−2.4** and finished around **−10.5**, episode length ranging 362–663. **Videos:** the robot *can* complete the task in low-disruption windows, but when the human gets very close it misses the grasp (e.g., the cabinet handle) and then drifts away from the workspace. This is the **unconstrained baseline** the P3.1 Lagrangian must beat — it shows the policy has task competence but no proactive avoidance strategy. The reward decline reflects exactly that: more workspace-penalty time / fewer +1 successes as the human disrupts more aggressively across training. Single-variable comparison for P3.1: warm-start P3.1 from the stage-1 snapshot (not stage-2) so the only difference is whether Q_c + λ + dual-Q is active. See decision log 2026-05-23.
- [ ] **Un-park P3.1.** Merge G1 into the phase-3 branch (see "Next session — start here"), then warm-start P3.1 from the stage-1 snapshot. NOTE: `train_cqn_as` `+snapshot_path` restores weights only (step counter + replay restart), so warm-starts train a fresh budget from the resumed weights.
