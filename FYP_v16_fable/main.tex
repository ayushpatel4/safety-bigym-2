...
rollouts of each task's unconstrained baseline policy under the
...
Configuration is managed with Hydra. W\&B is used for experiment
tracking, and per-run JSONL metric dumps provide W\&B-independent
reproducibility. Training targets a single NVIDIA A100 40\,GB GPU, and
every script has a CPU-only smoke gate.
...
Table~\ref{tab:filter-taxonomy} summarises the taxonomy. The claim is
...
\draw[black!45]
  ([yshift=-14mm]interm.south west) -- ([yshift=-14mm]persist.south east);
\foreach \p/\lab in {0/0\%,0.25/20\%,0.5/40\%,0.75/60\%,1/80\%} {
  \draw[black!45] ($(interm.south west)!\p!(persist.south east)+(0,-14mm)$)
    -- ++(0,-1.2mm)
    node[below,font=\scriptsize] {\lab};
}
\node[font=\scriptsize, below=7mm of interm.south] {low gate activity};
\node[font=\scriptsize, below=7mm of persist.south] {high gate activity};
...
not. Only a Power-and-Force-Limiting collision brake
(Section~\ref{sec:fw:pfl}) could address that tail. The regime
...
\chapter{A \coworker{} Patrol Episode, Frame by Frame}
\label{app:patrol-episode}

Section~\ref{sec:bench:coworker:traj} describes the three \coworker{}
trajectory modes in the abstract. This appendix shows what the dominant
