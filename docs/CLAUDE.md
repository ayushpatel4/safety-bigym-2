...
- **Phase 3 (P3.1) is DONE; E3.1/E3.2/E5 are launch-ready** (2026-05-31 — supersedes the two "P3.1 NOT done" / "`agent.update()` does not consume cost yet" notes above). `LagrangianCQNASAgent` (`agent=cqn_as_lagrangian`) trains Q_c + PID λ + dual-Q `argmax_a[Q_r − λ·Q_c]`, consuming `batch["cost"]`. Three load-bearing facts:
  ...
  - **The G1 SVF filter is noisy-native.** `checkpoints/svf_coworker_train_g1_0p3.pt` at **R=2.25** (`snapshots.py::SVF_FILTER_THRESHOLD_R`, the dense-0.3 m-sweep knee — `phase2_results.md` §0; the old `4.0` was the SMPL-H point). On **`oracle`** obs its Q **collapses** (`mean_q≈0.016 ≪ R` → 100% intervention, task frozen). So the **E4.1 headline (`run_e4_1_headline.sh`) and E4.3 (`run_e4_3_internalisation.sh`) eval on `noisy`** — the launchers/driver read R from `snapshots.py` and default `OBS_MODE=noisy`. The in-training `FILTER_PASSIVE` hook is useless (P3/P4 train oracle); do E4.3 post-hoc. Pipeline tools (all on `phase3`): `svf_sweep_g1_v1_baseline.sh`, `run_e3_1/2_*.sh` + `analyze_e3.py`, `run_e4_1_headline.sh` + `aggregate_e4_1.py`, `run_e4_3_internalisation.sh`, `aggregate_e5_1.py`.
