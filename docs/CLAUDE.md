...
- **The vendored C51 projection has a local index-clamp bugfix** (as of 2026-05-21). `C2FCritic.compute_target_q_dist` (`agents/cqn_as/agent.py`) now clamps `b = (Tz−v_min)/delta_z` to `[0, atoms-1]` before floor/ceil. Without it, float32 rounding at the support ceiling (`Tz==v_max`, hit by reward=1 success transitions) made `ceil(b)=atoms` → `index_add_` OOB → opaque CUDA device-side assert (`dstIndex < dstAddDimSize`), surfacing stochastically (more with more successful demos). This is the one sanctioned edit to the vendored `agent.py` (logged in its header). `train_cqn_as` also guards batch reward/discount/action/cost with `torch.isfinite` so a future NaN is a legible error, not an async CUDA assert. To debug such asserts: `CUDA_LAUNCH_BLOCKING=1` pins the real op.
