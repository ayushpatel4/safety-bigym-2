...
- **Phase 3 (P3.1) is DONE; E3.1/E3.2/E5 are launch-ready** (2026-05-31 — supersedes the two "P3.1 NOT done" / "`agent.update()` does not consume cost yet" notes above). `LagrangianCQNASAgent` (`agent=cqn_as_lagrangian`) trains Q_c + PID λ + dual-Q `argmax_a[Q_r − λ·Q_c]`, consuming `batch["cost"]`. Three load-bearing facts:
  ...
  - **The G1 SVF has an action de-normalisation bug — P2 RE-DO REQUIRED (2026-06-01).** `svf_collect`'s `_CQNASSnapshotPolicy` de-normalised the agent's `[-1,1]` action via `env.action_space` instead of the agent's **demo-derived** stats (how it deploys), so `checkpoints/svf_coworker_train_g1_0p3.pt` was trained on a mis-de-normalised policy. At runtime `benchmark_policy` (correct de-norm) feeds the critic OOD actions → `mean_q≈0.02` → **~100% intervention on BOTH oracle and noisy** (the earlier "oracle-collapse / noisy-native" story was wrong; R=2.25 and `phase2_results.md §0` are **superseded**). Fix landed: shared `env_adapter.action_stats_from_actions` (single source of truth) + the snapshot policy now de-normalises with demo stats (`tests/test_action_stats.py`). **Re-do P2: `scripts/run_p2_recollect_g1.sh`** (preflight → re-collect → retrain `svf_coworker_train_g1_0p3_v2.pt` → re-sweep), then update `snapshots.py` (new checkpoint + new R). E4.1 rows 1–3 (policy-only) and e3_1/e3_2 are unaffected. Pipeline tools (all on `phase3`): `svf_sweep_g1_v1_baseline.sh`, `run_e3_1/2_*.sh` + `analyze_e3.py`, `run_e4_1_headline.sh` (default `OBS_MODE=noisy`) + `aggregate_e4_1.py`, `run_e4_3_internalisation.sh`, `aggregate_e5_1.py`.
